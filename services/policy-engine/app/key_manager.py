#!/usr/bin/env python3
"""
RansomEye v1.0 Policy Engine - Key Manager
AUTHORITATIVE: Keypair generation and management for Policy Engine command signing
Python 3.10+ only
...
class PolicyEngineKeyManager:
    """
    Key manager for Policy Engine command signing.
    
    PHASE 4: Uses ed25519 for command signing (replaces HMAC-SHA256).
    Separate trust root from TRE, HAF, Audit Ledger.
    """
    ...
    def __init__(self, key_dir: Path):
        ...
        self.private_key_path = self.key_dir / "policy-engine-signing-key.pem"
        self.public_key_path = self.key_dir / "policy-engine-signing-key.pub"
        self.key_id_path = self.key_dir / "policy-engine-signing-key.id"
