...
class RenderEngine:
    ...
    def _render_csv(self, incident_id: str, view_type: str, content_blocks: List[Dict[str, Any]],
                   incident_snapshot_time: Optional[str] = None) -> bytes:
        """
        Render CSV report with branding (full report for display).
        
        GA-BLOCKING: Branding is added as comment header, but this method returns full report.
        Use render_evidence_content() to get hashable content only.
        ...
        Returns:
            Full CSV report as bytes (includes branding)
        """
        # Get evidence content
        evidence_content = self._render_csv_evidence_only(incident_id, view_type, content_blocks, incident_snapshot_time)
        
        # Add branding comment header (presentation layer, outside hash domain)
        output = io.StringIO()
        output.write(f"# Generated by {Branding.get_product_name()}\n")
        output.write(f"# {Branding.get_evidence_notice()}\n")
        output.write("\n")
        output.write(evidence_content.decode('utf-8'))
        
        return output.getvalue().encode('utf-8')
    
    def _render_csv_evidence_only(self, incident_id: str, view_type: str, content_blocks: List[Dict[str, Any]],
                                 incident_snapshot_time: Optional[str] = None) -> bytes:
        """
        GA-BLOCKING: Render evidence content only (no branding, hashable).
        ...
        # Header row (stable field ordering)
        header_row = ['Incident ID', 'View Type', 'Incident Snapshot Time', 'Block ID', 'Source Type', 'Source ID', 'Content Type', 'Content Reference', 'Display Order']
        writer.writerow(header_row)
        
        # Data rows (stable field ordering)
        for block in content_blocks:
            writer.writerow([
                incident_id,
                view_type,
                incident_snapshot_time or '',  # GA-BLOCKING: Use incident snapshot time (not system time)
                block.get('block_id', ''),
