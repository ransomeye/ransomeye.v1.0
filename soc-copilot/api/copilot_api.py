#!/usr/bin/env python3
"""
RansomEye SOC Copilot - Copilot API
AUTHORITATIVE: Single API for SOC copilot queries with feedback storage
"""
...
_copilot_dir = Path(__file__).parent.parent
if str(_copilot_dir) not in sys.path:
    sys.path.insert(0, str(_copilot_dir))
...
class CopilotAPI:
    """
    Single API for SOC copilot queries.
    ...
    def store_feedback(
        ...
        try:
            ...
            raise CopilotAPIError(f"Failed to store feedback: {e}") from e
